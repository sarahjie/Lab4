...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        int temporary=lineScanner.nextInt();
        while(temporary<1||temporary>MAX_HIT_POINTS)
        {
            System.out.println("Sorry. Hit points must be between 1 and 50:");
            temporary=lineScanner.nextInt();
        }
        tempPokemon.hitPoints =temporary;
        System.out.println("Split fifty points between attack level and defense level");
        System.out.println("Enter your attack level (1-49):");
        temporary=lineScanner.nextInt();
        while(temporary<1||temporary>49)
        {
            System.out.println("Sorry. The attack level must be between 1 and 49:");
            temporary=lineScanner.nextInt();
        }
        tempPokemon.attackLevel =temporary;
        int complementTemp=50-temporary;
        System.out.println("Enter your attack level (1-"+complementTemp+"):");
        temporary=lineScanner.nextInt();
        while(temporary<1||temporary>complementTemp)
        {
            System.out.println("Sorry. The attack level must be between 1 and "+complementTemp+":");
            temporary=lineScanner.nextInt();
        }
        tempPokemon.defenseLevel =temporary;
        return tempPokemon;
    }
